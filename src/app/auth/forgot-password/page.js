  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { error: resetError } = await supabase.auth.resetPasswordForEmail(email, {
        // ✅ FIX: punta a /auth/reset-password
  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 p-8">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">Ti invieremo un link per resetarla</p>
      </div>
      ...
      {success ? (
          ...
          <div className="p-6 bg-green-50 border border-green-200 rounded-xl">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-green-700">Abbiamo inviato un link a:</p>
                <p className="text-sm font-semibold text-green-900 mt-1">{email}</p>
              </div>
            </div>
          </div>
          <Link href="/auth/login" className="block w-full py-3.5 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition text-center">
            Torna al Login
          </Link>
        </div>
      ) : (
        <>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            >
              {loading ? <><Loader2 className="animate-spin" size={20} /><span>Invio...</span></> : <span>Invia Link di Reset</span>}
            </button>
          </form>
        </>
      )}
