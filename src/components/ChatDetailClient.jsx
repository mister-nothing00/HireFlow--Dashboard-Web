...
import { supabase } from '@/lib/supabase'; // ✅ CLIENT supabase, non server
  ...
  return (
    <div className={`flex ${isOurs ? 'justify-end' : 'justify-start'} mb-3`}>
      <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl ${
        ...
        <p className={`text-xs mt-1 ${isOurs ? 'text-blue-200' : 'text-gray-400'}`}>
          {time}
        </p>
      </div>
    </div>
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        setMessages(prev => {
          // Dedup: evita doppio da optimistic + realtime
  ...
  const handleSend = useCallback(async (e) => {
    ...
    try {
      const { data, error } = await supabase
        .from('messages')
        .insert([{ match_id: match.id, sender_id: company.id, sender_type: 'company', content }])
        .select()
        .single();
  ...
  return (
    <div className="flex flex-col h-screen bg-white">
      ...
      <div className="flex items-center gap-4 px-6 py-4 border-b border-gray-200 bg-white shadow-sm">
        <Link href="/dashboard/chat" className="text-gray-500 hover:text-gray-700 transition">
          <ArrowLeft size={22} />
        </Link>
      ...
      {/* Input */}
      <form onSubmit={handleSend} className="flex gap-3 px-6 py-4 border-t border-gray-200 bg-white">
        <input
          type="text"
          value={newMessage}
